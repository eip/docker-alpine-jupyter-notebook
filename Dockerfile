...
RUN echo "@testing http://dl-cdn.alpinelinux.org/alpine/edge/testing" >> /etc/apk/repositories && \
    ...
    apk add --update tini python3 libstdc++ openblas@testing freetype wget ca-certificates && \
    python3 -m ensurepip && rm -r /usr/lib/python*/ensurepip && \
    pip3 install --upgrade pip setuptools && \
    apk add --update --virtual .devdeps@testing python3-dev make cmake clang clang-dev g++ linux-headers libtbb@testing libtbb-dev@testing openblas-dev@testing freetype-dev && \
    ...
    python3 setup.py build -j 4 install &> /opt/numpy.log && echo "Successfully installed numpy" && \
    cd /opt/tmp && \
    ...
    cmake -D CMAKE_BUILD_TYPE=RELEASE \
      ...
      -D BUILD_TESTS=NO .. &> /opt/opencv.log && \
    make >> /opt/opencv.log 2>&1 && make install >> /opt/opencv.log 2>&1 && echo "Successfully installed opencv" && \
